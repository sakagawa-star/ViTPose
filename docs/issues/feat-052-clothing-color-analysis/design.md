...
- **ADR-2 色相環対応**: 採用＝循環平均中心の相対角度パーセンタイル。却下＝固定シフト（H>90→H-180、赤専用ヒューリスティック）。理由: 循環平均は任意色相（赤・青・緑）で数学的に正しく中心化でき、将来の青・白患者対応（Could）の布石になる。事前実測の固定シフトはピンクでは同結果だが汎用性を欠く
