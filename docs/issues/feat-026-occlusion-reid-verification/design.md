...
- **Event Log の全件出力**: 321Kフレームでもイベントは数百件程度のため全件出力する。イベント数が1000件を超えることは室内動画では想定しにくい
...
2. **誤同定の防止**: 全体モード描画動画で、同一フレーム内の異なる人物が異なる色で描画されているか。特に看護師等の入退室時に対象のstable_idが割り当てられていないか
