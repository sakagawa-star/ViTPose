...
`scripts/convert_pink_to_blue_video.py` を新規作成する。既存のピンク対象動画（病院着がピンクの研究員撮影データ）の HSV 空間でピンク領域を青に置換し、合成「青対象」テスト動画を生成する。NDA により本物の青対象動画を入手できないため、青色対応パイプライン（feat-045 以降）の動作検証用の合成データソースとして利用する。

## なぜ作るのか

- 本物の青対象動画は NDA により入手不可。青色対応パイプラインの開発・検証に使う代替データが必要
- ピンク対象動画は既に `experiments/input/camSony1_L.mp4` / `testdata/camSony1_S.mp4` 等で利用可能で、`postprocess_pink_id.py` の HSV 範囲でピンク領域を高精度で抽出できる
...
Blue3.png（背景込みの実対象画像）の HSV 分析で確認された事項:

- 病院のシーツ・寝具に**青系のチェック柄**が含まれる場合があり、HSV 値が病院着とほぼ同じ範囲（H≈110、S≈40）に収まる
- 単純な HSV 範囲指定（例: `H=90-140, S=10-80, V=80-230`）だけでは「対象の服 vs シーツ」を分離できない可能性が高い
...
2. **BB 内のみで判定**: 検出器（YOLO11x）が対象を BB として検出している前提で、BB 内の青比率のみを評価。シーツが BB 全体に入り込まない限り、対象領域中心の評価になる
