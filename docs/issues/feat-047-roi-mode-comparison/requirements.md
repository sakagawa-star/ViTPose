...
- **δ（不一致目視）**: 「BB モードで pink_id=1 になった人物」と「keypoint-rect モードで pink_id=1 になった人物」が異なるフレームを抽出し、目視で「どちらが正しい対象を選んでいるか」を判定

「擬似正解」を立てず、不一致フレームのみを人間が目視判定することで、循環参照を避けた信頼性のある評価ができる。

### 1.3 誰が使うのか

本プロジェクトの開発者（feat-046 の効果検証、ピンク対象 → 青対象拡張の判断材料を集める者）。
