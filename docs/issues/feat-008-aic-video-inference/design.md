...
- **出力**: `output/feat-008/vis_cam05520129.mp4`（mp4v コーデック、1920x1080, 30fps）
...
デモスクリプト内部の処理フロー（feat-004/006と同一のスクリプト）:
...
5. 各フレームに対してループ:
   ...
   c. `inference_top_down_pose_model()` でポーズ推定（14キーポイント）
...
uv run python -c "
import cv2
cap = cv2.VideoCapture('output/feat-008/vis_cam05520129.mp4')
...
- CUDA out of memory: フレームごとの推論であるためOOMが発生する可能性は低い
- 動画ファイルが開けない場合: `isOpened()` が False となりassertで停止
...
- **採用案**: `demo/top_down_video_demo_with_mmdet.py`（feat-004/006と同一）
- **理由**: AICもtop-downパイプラインで推定可能。dataset_infoを設定ファイルから読み取るため同じスクリプトが使える
