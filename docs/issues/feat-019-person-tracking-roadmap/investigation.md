...
HALPE 26パイプラインに人物トラッキング機能を追加し、フレーム間で同一人物にIDを維持する。トラッキングIDをOpenPose JSONに記録し、後処理で「最も長時間映っているID＝対象」として特定可能にする。

## 2. 要件

### 2.1 動画の特性

- 室内の固定カメラ映像（1時間単位のファイル、1週間連続記録）
- 通常1人の対象＋看護師等が出入りして2-3人になることがある
- 対象はてんかん対象でベッド上で自由に動く（位置固定ではない）
- 対象がカメラから見切れることがある（数秒以上）
...
- 動画間（ファイル跨ぎ）のID継続は不要（各動画で独立に対象を特定する）
...
- Re-IDモデルはMOT17（街中の歩行者）で学習されており、室内環境（病院着、臥位）でのRe-ID精度は未知
- 追加のGPUメモリ消費（Re-IDモデル分）

**結論**: ~~要件を満たす唯一の手法。ただしRe-IDの室内環境での精度は技術検証が必要。~~ → BoxMOT + Deep OC-SORTの発見により、より優れた選択肢が見つかった（3.5節参照）。
...
| 3 | Deep OC-SORT室内動画検証 | 室内動画（testdata/cam05520129.mp4）でDeep OC-SORTを実行し、トラッキング精度を目視確認 | 2 | Re-IDが室内環境で機能するか確認 |
| 4 | 見切れ再同定の検証 | 対象が見切れる場面でIDが維持されるか確認。パラメータ調整 | 3 | 数秒の見切れ後の再同定精度を確認 |

**Phase 5Aの判定基準**:
- Deep OC-SORTが室内動画で実用的なトラッキング精度を示す → Phase 5Bへ進む
...
| 順番 | 案件案 | 概要 | 依存 | 目的 |
|:--:|--------|------|------|------|
| 8 | 対象ID特定スクリプト | JSON群からトラッキングIDごとの出現フレーム数を集計し、最長時間のIDを対象として出力 | 6 | 対象のトラッキングIDを自動特定 |
| 9 | 対象フィルタリング | 指定したトラッキングIDのキーポイントのみを抽出したJSONを出力 | 8 | 対象のキーポイントのみを後段に渡す |

### 実装順序の根拠

- **Phase 5A（技術検証）を最優先**: Deep OC-SORTの室内動画での精度が未知であり、精度不足なら以降のPhaseは不要。最小コストで判断材料を得る
...
| リスク | 影響 | 対策 |
|--------|------|------|
| Re-IDが室内環境で精度不足 | 見切れ後の再同定失敗 | BoxMOT内の他Re-IDモデル（CLIPReID等）に変更、または他トラッカー（StrongSORT, BoTSORT）を試す |
