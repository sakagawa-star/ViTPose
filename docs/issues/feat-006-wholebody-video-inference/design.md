...
- **出力**: `output/feat-006/vis_cam05520129.mp4`（mp4v コーデック、1920x1080, 30fps）
...
デモスクリプト内部の処理フロー（feat-004と同一のスクリプト）:
...
5. 各フレームに対してループ:
   ...
   c. `inference_top_down_pose_model()` でポーズ推定（133キーポイント）
   d. `vis_pose_result()` でフレームにキーポイント＋スケルトンを描画
   e. `videoWriter.write()` で描画済みフレームを出力動画に書き込む
6. `cap.release()` / `videoWriter.release()` でリソース解放

出力ファイル名は `vis_{入力動画のbasename}`（105行目: `f'vis_{os.path.basename(args.video_path)}'`）で自動生成される。
...
uv run python -c "
import cv2
cap = cv2.VideoCapture('output/feat-006/vis_cam05520129.mp4')
...
- CUDA out of memory: WholeBody 133キーポイントはCOCO 17より出力チャネル数が多いが、フレームごとの推論であるためVRAM使用量の増加は微小。OOMが発生する可能性は低い
- 動画ファイルが開けない場合: `cv2.VideoCapture` の `isOpened()` が False となりassertで停止。動画パスを確認する
- 途中でフレーム読み込みが失敗した場合: `flag` が False になりループが正常終了

#### 境界条件

- 特定フレームで人物が検出されない場合: キーポイントなしのフレームが出力される
...
- **採用案**: `demo/top_down_video_demo_with_mmdet.py`（feat-004と同一）
- **理由**: WholeBodyもtop-downパイプラインで推定可能。dataset_infoを設定ファイルから読み取るため、COCO用とWholeBody用で同じスクリプトが使える
