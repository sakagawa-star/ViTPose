# feat-036: postprocess_patient_id.py 実装（pink_id + track_id ハイブリッド対象追跡）
...
- 要求 A（種による直接判定）: pink_id=1 で対象を直接判定 → 動作確認済み
- 要求 B（track_id 拡張）: pink_id 非観測フレームで track_id 経由の伝播 → camSony1_S で +4 フレーム、camSony1_L で +13,456 フレームの対象追加を確認
...
- **種（pink_id=1）**: 色ベースで対象を直接判定する情報源。これ自体が対象シグナル
- **拡張（track_id）**: 種が観測できないフレームに対象ラベルを時間方向へ伝播させる道具。単独では対象を決定できない
...
4. 上記いずれにも該当しない → `pink_track_id = -1`（非対象）

判定 2（種）と判定 3（拡張）の順序は重要である。`pink_id=1` は色ベースの直接シグナルなので、`track_id` の状態に関わらず常に対象と判定される。`track_id` は種が付いた後に他フレームへ対象ラベルを伝播させる手段として働く。
...
これにより、`track_id` が全区間で一度でも有効な `pink_id=1` と紐づいていれば、その `track_id` を持つ全フレームの BB が遡って `pink_track_id=1` となり、pink が観測できない前後のフレームでも対象追跡が継続する。
