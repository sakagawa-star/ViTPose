...
feat-052（服色特徴量分析ツール）の調査で、本番患者の淡いピンク服が、テスト動画由来でハードコードされた `FIXED_HSV_RANGES` とズレており pink_ratio が取りこぼされる（実例 `E0014-01.png` で current pink_ratio=0.0099、推奨レンジでは 0.6046）ことが判明した。推奨レンジの算出ツールはできたが、それを `postprocess_pink_id.py` に反映する手段がソースコードのハードコード編集しかなく、患者ごとの差し替えができない。
