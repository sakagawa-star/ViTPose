...
│   ├── postprocess_pink_id.py        # Pink-idポストプロセス：JSONにpink_id付与（feat-033、feat-053で--hsv-config対応）
...
│   ├── analyze_clothing_color.py    # 服色特徴量分析・HSVレンジ提案ツール（feat-052、静止画→ViTPose胴体ROI→推奨FIXED_HSV_RANGES。feat-054で推奨レンジを--hsv-config互換JSON出力）
