...
- Stage2 track_id 付与: 既存 feat-035 `postprocess_track.py`（完了済み）
- Stage3 pink_id 付与: 既存 feat-033 `postprocess_pink_id.py`（変更なし、生 dict 保持設計により Stage 2 の `track_id` は自動通過）
- Stage4 pink_track_id 算出: **feat-036**（新規 `postprocess_pink_track_id.py`、pink_id + track_id ハイブリッド結合、着手可能）

- **feat-036**: `postprocess_pink_track_id.py` 実装（要求仕様書作成前）
